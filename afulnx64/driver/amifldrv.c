...
if ((major=wrap_register_chrdev()) < 0) { return (-EIO); }
...
if (kmalloc_ptr){pvArg0 = kmalloc_ptr; wrap_kfree();}}
} kcount = 0; }
ulArg0 = major;
...
for (virt_addr=(unsigned long)kmalloc_area; virt_addr<(unsigned long)kmalloc_area+kmalloc_len; virt_addr+=PAGE_SIZE)
{ulArg0 = virt_addr; pvArg0 = wrap_virt_to_page(); wrap_mem_map_reserve();}
for (i=0; i<(kmalloc_len/sizeof(int)); i++){kmalloc_area[i]=(0xafd0<<16) +i;}
...
if (kmalloc_ptr){
for(virt_addr=(unsigned long)kmalloc_area; virt_addr<(unsigned long)kmalloc_area+kmalloc_len;virt_addr+=PAGE_SIZE)
{ulArg0 = virt_addr;pvArg0 = wrap_virt_to_page();wrap_mem_map_unreserve();}
if (kmalloc_ptr) {pvArg0 = kmalloc_ptr;wrap_kfree();}
kmalloc_len = 0L;kmalloc_ptr = NULL;kmalloc_area = NULL;
kcount--;
if (isearch != kcount) {
...
if (!kmalloc_ptr){return(-ENXIO);}
if (size>kmalloc_len){return(-ENXIO);}
if ((offset+size)>kmalloc_len){return -ENXIO;}
...
ulArg2 = size;
pgArg0 = PAGE_SHARED;
if (wrap_remap_page_range()){return -ENXIO;}
return(0);
}
